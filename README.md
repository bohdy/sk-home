...
Use the repo-local `sk-home-orchestrator` skill for scoped repository changes. It sizes each task before delegating. Light reversible changes stay with the root agent or use one reviewer. Standard changes use planner, coder, and reviewer, adding documenter when a separate documentation pass is needed. Infrastructure and security changes use the full applicable flow, including firewaller and final review. The orchestrator records skipped roles and escalates when scope or risk grows.

The orchestrator uses zero automatic repair cycles for light work, one for standard work, and at most three for infrastructure, security, deployment, or external-system work. It keeps detailed role reports in Codex threads and prepares a draft pull request only after the required reviews and repository checks pass. Full-risk work always runs the firewaller, which records either proposed rule changes or an explicit no-change result. For infrastructure work, the orchestrator preserves the existing firewall inventory, immutable-plan, and GitHub `production` approval boundaries. It can dispatch an existing production workflow only after the change is merged to `main`, the evidence matches that merge, and the user gives explicit in-chat approval.
...
Review `network-gw-unifi-ap-tofuplan` and require exactly three non-noop creates on initial recovery for the AP inform, STUN, and discovery rules. Reject deletes, replacements, and unrelated resources. The targeted recovery does not target or apply the broader Kubernetes VIP policy; that separate full firewall plan owns the declarative VLAN 102 exclusion. Until that plan is applied, VLAN 102 must remain outside live `LAN` and broad Kubernetes VIP access. The AP rules allow only that subnet entering `vlan102` to the UniFi device-communication VIP `10.1.30.1` on TCP/8080, UDP/3478, and UDP/10001. After review, run a separate dispatch with `-f apply_gateway_unifi_ap=true -f plan_gateway_unifi_ap=false`; that trusted run regenerates and uploads its own immutable artifact from current `main`, so inspect the new artifact and approve the production gate only when it passes the same exact three-create guard. The production-gated job applies that artifact and verifies enabled rules, sanitized fields, and order before both enabled forward deny anchors. DHCP may advertise `10.1.30.53` to VLAN 102, but it does not grant DNS or general Kubernetes VIP forwarding; only the three UniFi controller ports are allowed. Re-run the review-only dispatch afterward and require an empty plan. If verification fails, use RouterOS Safe Mode or the local console to disable only the three new stable-comment rules, correct the declaration, and create a fresh reviewed artifact; do not use REST or the broader firewall path as rollback.
...
gh workflow run terraform.yaml --ref main \
  -f apply_gateway=false \
  -f apply_gateway_qdevice=false \
  -f plan_gateway_qdevice=true
```

For a real apply, dispatch `apply_gateway_qdevice=true`; the run first pauses at a `production` gate, applies the reviewed initial qdevice artifact for the provider-managed network/config/key resources and state-only desired records, then creates the native RouterOS 7.23 mount/container objects and uploads a fresh immutable qdevice plan. A second `production` gate then applies only that fresh artifact. A plan-only dispatch remains read-only on RouterOS and its artifact is not reused by a later dispatch:

```bash
gh workflow run terraform.yaml --ref main \
  -f apply_gateway_qdevice=true \
  -f plan_gateway_qdevice=false
```

The workflow applies only that immutable artifact, then verifies the qnetd container, persistent mounts, VLAN 100 attachment, the authorized-key file, and qnetd/5403. The pinned RouterOS provider sends `name` for container mounts and cannot represent the RouterOS 7.23 native `list`/`mountlists` fields, so OpenTofu keeps state-only desired records for those two objects and the first gated phase uses the existing secure REST helper to reconcile them idempotently. This is the documented, narrowly scoped API exception; it refuses legacy provider state, conflicts, deletes, replacements, and unrelated plan changes. If the live container list is empty, the reviewed initial artifact may migrate the global RouterOS extraction paths to the declared qnetd paths; it never adopts arbitrary paths or deletes old directories, and the job rechecks the empty-container condition immediately before applying that migration. Before any production apply, run the repository's fresh read-only firewall inventory and confirm VLAN 100 remains the intended boundary. The committed cluster key is fingerprinted as `SHA256:9x+cumin8611j5uKUYwq2kTRI6nVbTtxF9GoeISjeF8`; update the public-key file and this guard together if the Proxmox cluster key is intentionally rotated. This path does not add firewall rules because the qdevice and Proxmox nodes share the management VLAN 100 Layer-2 segment. Before enabling it, format the USB drive with a RouterOS-supported filesystem and create these exact directories: `usb1/qnetd`, `usb1/qnetd/root`, `usb1/qnetd/layers`, `usb1/qnetd/tmp`, `usb1/qnetd/nssdb`, `usb1/qnetd/ssh-host-keys`, and `usb1/qnetd/ssh-authorized-keys`; these are one-time RouterOS prerequisites and are intentionally not destructive OpenTofu resources. If preflight reports conflicting extraction paths or a conflicting/non-qnetd container, including a stopped one, inspect live `/rest/container` and `/rest/container/config` state instead of manually changing paths or blindly retrying. Run the review-only dispatch again afterward and require an empty plan.
