...
variable "site_name" {
  description = "Logical site name used by the Switch 1NP interfaces stack."
  type        = string
  default     = "primary"
}
...
variable "ethernet_interfaces" {
  description = "Physical Ethernet interface settings managed on Switch 1NP."
  type = map(object({
...
variable "bridge" {
  description = "Single bridge definition managed on Switch 1NP."
  type = object({
    name           = string
    comment        = optional(string)
...
variable "bridge_ports" {
  description = "Switch 1NP bridge-port membership and ingress behavior keyed by physical interface name."
  type = map(object({
    comment           = optional(string)
...
variable "bridge_vlans" {
  description = "Switch 1NP bridge VLAN table entries keyed by logical VLAN record name."
  type = map(object({
    comment  = optional(string)
    vlan_ids = set(string)
    tagged   = set(string)
...
variable "vlan_interfaces" {
  description = "Switch 1NP VLAN interfaces keyed by RouterOS interface name."
  type = map(object({
    comment   = optional(string)
    interface = string
    vlan_id   = number
