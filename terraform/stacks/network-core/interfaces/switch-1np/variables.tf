...
variable "bridge_ports" {
  description = "Switch 1NP bridge-port behavior and explicit VLAN membership keyed by physical interface name."
  type = map(object({
    comment           = optional(string)
...
variable "device_vlans" {
  description = "Switch 1NP VLAN behavior keyed by the shared VLAN catalog key."
  type = map(object({
