...
This root manages `GW` interface topology and interface metadata in its own
Terraform state so gateway changes do not share a blast radius with switch
imports or switch VLAN edits.
...
This root owns the gateway-specific interface concerns that should be planned
...
The shared non-secret gateway interface configuration is committed in
...
- This root manages objects that already exist on the live gateway. Import the
  existing bridge, bridge ports, bridge VLANs, VLAN interfaces, and `sit1`
  before the first apply.
- Keep gateway bridge ports and bridge VLAN entries synchronized in the same
  change whenever a port role changes.
