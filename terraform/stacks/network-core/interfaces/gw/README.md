...
The shared non-secret gateway interface configuration is committed in `interfaces.auto.tfvars`. Use `terraform.tfvars.example` only for local-only overrides or temporary inputs that should not become shared desired state.
...
- Keep gateway bridge ports and bridge VLAN entries synchronized in the same change whenever a port role changes.
