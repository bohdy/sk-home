...
Each per-device root commits its own non-secret `interfaces.auto.tfvars` file. Use the root-specific `terraform.tfvars.example` files only for local-only overrides or temporary inputs that should not become shared desired state.
