# Expose the resolved stack context so the Switch 1PP interface root can be
# reviewed before operators point it at the live RouterOS device.
output "stack_context" {
  description = "Resolved root-module context for the Switch 1PP interfaces stack."
  value = {
...
output "mikrotik_device" {
  description = "Configured MikroTik Switch 1PP managed by the interfaces stack."
...
output "interface_inventory" {
  description = "Committed non-sensitive Switch 1PP interface inventory managed by the interfaces stack."
