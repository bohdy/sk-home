...
# on Switch 1PP so each apply only touches the VLANs it actually carries.
variable "bridge_vlan_keys" {
  description = "Shared VLAN catalog keys that should have bridge VLAN table entries on Switch 1PP."
  type        = set(string)
  default     = []
}
