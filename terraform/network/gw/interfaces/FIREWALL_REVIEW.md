...
The AP management exception is intentionally separate from the broader hardening plan. The broad Kubernetes service-VIP rule explicitly excludes source subnet `10.1.102.0/24`, so APs cannot use it to reach arbitrary VIPs or ports. The dedicated rules permit only that source subnet on ingress interface `vlan102` to the UniFi device-communication VIP `10.1.30.1`: TCP/8080 for inform, UDP/3478 for STUN, and UDP/10001 for discovery. The three stable `sk-firewall/forward/allow-unifi-ap-*` rules are sequenced immediately after `sk-firewall/forward/allow-kubernetes-service-vips` and before `sk-firewall/forward/drop-inter-vlan` and `sk-firewall/forward/drop-unmatched`; established/related handling provides stateful return traffic.

Review and apply this exception only through the mutually exclusive `plan_gateway_unifi_ap` and `apply_gateway_unifi_ap` inputs from `main`. The review-only run produces `network-gw-unifi-ap-tofuplan`; the apply run regenerates and uploads its own immutable artifact from current `main`, so inspect that same-run artifact and approve the production gate only when the exact guard passes. The guard accepts exactly the three AP-rule creates plus the one broad Kubernetes VIP exclusion update on first apply, or an empty plan after recovery. The production-gated apply verifies exact enabled rule fields, the exclusion, and order through sanitized metadata only. This path does not add VLAN 102 to `LAN`, add NAT, expose other controller ports, or permit general Kubernetes VIP/DNS or inter-VLAN access.
...
| VLAN 102 | DHCP may advertise Kubernetes DNS, but the broad Kubernetes VIP rule excludes this source; no general Kubernetes VIP or DNS forwarding is permitted. | Exact routed rule to the NAS host remains a separate existing policy. | Only TCP/8080, UDP/3478, and UDP/10001 to UniFi VIP `10.1.30.1` for AP management; the SNMP reply path remains owned for worker polling. | Not added to `LAN`; no general WAN egress or broad inter-VLAN access. |
...
3. The plan places IPsec exceptions before FastTrack, creates exact Synology rules for routed VLANs 10, 20, 101, and 102, adds the exact Kubernetes and VLAN 10 management API paths for both node addresses, preserves VLAN 100 local-L2 access, allows DHCP on every addressed VLAN, and constrains Kubernetes service VIP forwarding to the dedicated internal interface and address-list boundaries. VLAN 102 is excluded from the broad VIP rule and receives only the explicit UniFi controller ports; a DHCP-advertised DNS address is not forward permission. The plan also excludes the SMTP relay VIP from the general service rule and all Kubernetes VIPs from the broad road-warrior LAN rule.
...
6. Verify DHCP leases and advertised DNS separately from forwarding: VLAN 102 may receive the Kubernetes DNS address through DHCP, but DNS and other Kubernetes VIP forwarding from VLAN 102 must be denied; only TCP/8080, UDP/3478, and UDP/10001 from APs to `10.1.30.1` may pass. Verify the other addressed VLANs' explicit Kubernetes VIP/DNS paths, gateway DHCP/DNS and management paths, BGP from all six declared nodes, worker-to-Synology and worker-to-UniFi SNMP, printer-to-relay SMTP, trusted WAN egress, existing WAN destination NAT, both WireGuard listeners, the preserved IPv6 peer paths, and denial of unknown WAN, inter-VLAN, BGP, WireGuard, and SMTP sources.
