...
The pre-apply targeted hardening plan reported 24 additions, 10 in-place changes, and 0 deletions. It creates the dedicated internal-VLAN interface boundary, five internal-network address-list entries, one missing VLAN 10 management entry, four exact routed Synology rules, the Kubernetes-to-Proxmox exporter exception, ownership checks, and the IPv6 filter/NAT ordering resources; it disables the five broad legacy VLAN/NAS rules and updates the Kubernetes service-VIP, DHCP, WireGuard destination boundary, and filter ordering. The broader hardening plan remains unapplied; the separate exporter recovery created only its one firewall rule.
