...
The pre-apply targeted hardening plan reported 23 additions, 8 in-place changes, and 0 deletions. It creates the dedicated internal-VLAN interface boundary, five internal-network address-list entries, one missing VLAN 10 management entry, four exact routed Synology rules, the Kubernetes-to-Proxmox exporter exception for `10.1.100.201`, ownership checks, and the IPv6 filter/NAT ordering resources; it preserves the shared-NAS path for every internal VLAN, keeps the VLAN 10 management and ping exceptions active, and updates the Kubernetes service-VIP, DHCP, WireGuard destination boundary, and filter ordering. The broader hardening plan remains unapplied; the separate exporter recovery created only its one firewall rule, and the dedicated ether7 workflow adds the matching Kubernetes TCP/8006 rule for `10.1.100.202` plus identical VLAN 10 management rules for both API endpoints.
...
The input chain places established/related handling, invalid-state protection, loopback, DHCP from the dedicated `sk-internal-vlans` interface list, DNS, ICMP, IPsec, BGP, SNMP, and management before the canonical terminal drop. The active runner and WireGuard handshake rules are also before that drop. The VLAN 10 management and ping forward exceptions remain active before the forward-chain terminal drop, while unrelated imported legacy/default rules remain state-owned after their terminal boundary.

The forward chain places IPsec policy handling before FastTrack, then established/related and invalid-state handling, the shared internal-to-Synology path plus exact routed Synology rules for VLANs 10, 20, 101, and 102, trusted-LAN WAN egress, active site/WAN/WireGuard paths, Kubernetes service VIP access from the dedicated internal interface and address-list boundaries, WireGuard DNS, SMTP, SNMP, the exact Kubernetes-to-Proxmox API exceptions for `10.1.100.201` and `10.1.100.202`, explicit management exceptions, destination NAT, and the inter-VLAN/WAN/unmatched terminal drops. The general Kubernetes VIP rule excludes the printer-only SMTP VIP, and the road-warrior LAN rule excludes the full Kubernetes VIP range so the dedicated DNS rules remain meaningful. The internal NAS rule and the VLAN 10-to-VLAN 100 management/ping exceptions remain active before the terminal drop; unrelated imported diagnostic and duplicate/default forward rules remain represented after it, and the generated FastTrack counter row is state-owned but not moved.

This ordering preserves the requested shared-NAS and VLAN 10 management paths while keeping FastTrack ahead of ordinary established traffic. The rule-order change alone is not a throughput proof; the acceptance tests below remain required.
...
| Order | Live comment or identity | Current effect and review | Implemented disposition |
| ---: | --- | --- | --- |
| 1 | `lan-to-nas` | Accepts every routed protocol to shared NAS `10.1.100.10`; the reviewed declaration now limits its source to the internal-VLAN address list and interface list, excluding WAN ingress while covering every addressed VLAN. It is placed after connection tracking so established traffic can use FastTrack. | Keep enabled and move before the terminal drops; retain the exact per-routed-VLAN rules as explicit ordered policy and verify counters and access from every VLAN. |
| 2 | `Allow VLAN10 to VLAN100 management` | Allows TCP/22, 80, 443, 445, 5000, and 5001 from all of VLAN 10 to all of VLAN 100. This is an intentional LAN management exception requested for the homelab. | Keep enabled and move before the terminal drops; retain its explicit source, destination, protocol, and port boundary. |
| 3 | `Allow VLAN10 to VLAN100 ping` | Allows ICMP from all of VLAN 10 to all of VLAN 100. This is an intentional LAN management/diagnostic exception requested for the homelab. | Keep enabled and move before the terminal drops; retain its explicit source and destination boundary. |
