...
The RouterOS container configuration is global, so the targeted workflow fails closed if any other container exists, if the existing extraction paths conflict with these USB-backed paths, or if the existing registry is not Docker Hub. Review this global configuration before enabling qnetd because changing it can affect every RouterOS container. The authorized key is the Proxmox cluster's public root SSH key, committed as public material and guarded by fingerprint `SHA256:9x+cumin8611j5uKUYwq2kTRI6nVbTtxF9GoeISjeF8`; no private key is needed by the qdevice workflow or placed in OpenTofu state or RouterOS.

Review and apply qdevice changes only from `main` through the mutually exclusive targeted workflow. A plan-only run adopts and validates the existing veth, bridge port, VLAN 100 row, and global container configuration, creates the immutable `network-gw-qdevice-tofuplan` artifact, and rejects deletes, replacements, or unrelated resources. An apply run creates a fresh artifact and pauses at the production environment gate; review that artifact before approving the gate, then run the review-only dispatch again and require an empty plan.
