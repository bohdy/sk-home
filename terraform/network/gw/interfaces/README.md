...
Enable it only after the USB drive is formatted with a RouterOS-supported filesystem and the parent paths `usb1/qnetd`, `usb1/qnetd/layers`, `usb1/qnetd/tmp`, `usb1/qnetd/nssdb`, `usb1/qnetd/ssh-host-keys`, and `usb1/qnetd/ssh-authorized-keys` exist as directories. Leave `usb1/qnetd/root` absent or empty; RouterOS creates that per-container root store during `/container/add`. The targeted workflow supplies `qdevice.ssh_public_key` from the committed `qdevice-authorized.pub` cluster key; direct stack callers must provide an equivalent public key through the protected variable injection path. It must be non-empty when `qdevice.enabled` is true. When qdevice is enabled, OpenTofu also creates a narrow forward-chain allow from the self-hosted runner `10.1.20.200` entering `vlan20` to the qdevice host on `vlan100` over TCP/5403, ordered before `sk-firewall/forward/drop-inter-vlan`. Ordinary gateway plans keep qdevice lifecycle resources disabled, while the dedicated full-firewall plan supplies the non-secret public key and preserves the existing qdevice firewall row as part of the audited live ownership boundary.

Recovery classifies RouterOS HTTP errors by status and sanitized error class without logging raw response bodies; final verification remains silent and fails closed on HTTP errors. Both runtime checks accept either RouterOS's `.running` representation or its human-readable `.status` representation when confirming that qnetd is running.

The RouterOS container configuration is global, so the targeted workflow fails closed if any other container exists, if a populated container uses extraction paths different from these USB-backed paths, or if the existing registry is not Docker Hub. When the live container list is empty, the reviewed initial artifact may declaratively migrate the global extraction paths and set the registry URL to Docker Hub for the qnetd pull; it never adopts arbitrary paths or deletes existing data. The recovery may remove only the exact empty `root-dir` placeholder because RouterOS requires that path to be absent before `/container/add`; it refuses to remove the placeholder if any descendant exists. The native container is created with the fixed RouterOS name `qnetd`, which is required by the `/container/add` command and checked during adoption and final verification. RouterOS exposes the provider-owned global extraction fields with a leading `/` (`/usb1/qnetd/layers` and `/usb1/qnetd/tmp`); native mount sources are declared as `usb1/...` but RouterOS reports them with one leading `/`, and REST may report the container `root-dir` the same way, so the workflow canonicalizes only those exact `/usb1/...` representations for comparison. File and directory checks use the declared `usb1/...` paths, and the workflow rejects empty, traversal, or non-USB extraction paths. Review this global configuration before enabling qnetd because changing it can affect every RouterOS container. The pinned RouterOS provider currently rejects its `routeros_container_mounts` `name` payload on RouterOS 7.23 and cannot safely own the container's native `mountlists` field. The initial reviewed artifact creates the provider-managed prerequisites and state-only desired records; that record also preserves the complete VLAN 100 tagged/untagged membership so preflight rejects unrelated port drift instead of rewriting it. The production-gated recovery script is the only owner of the two incompatible live objects, uses native `list`/`mountlists` fields, and never deletes them. An interrupted recovery may leave an exact subset of the reviewed native mount rows; the next reviewed recovery completes that subset and rejects any unrelated, duplicate, or mismatched row. The authorized key is the Proxmox cluster's public root SSH key, committed as public material and guarded by fingerprint `SHA256:9x+cumin8611j5uKUYwq2kTRI6nVbTtxF9GoeISjeF8`; no private key is needed by the qdevice workflow or placed in OpenTofu state or RouterOS.
...
| 5-6 | `sk-firewall/input/allow-dns-udp`, `sk-firewall/input/allow-dns-tcp` | Allow trusted `LAN` clients to use the gateway resolver on port 53; VLAN 101 uses the Kubernetes DNS VIP through the forward chain, while VLAN 102 is limited to its explicit AP controller path. |
...
| 15 | `sk-firewall/forward/allow-kubernetes-service-vips` | Allow internal-network sources on the dedicated internal VLAN interface list to Kubernetes VIPs, excluding AP source subnet `10.1.102.0/24` and SMTP relay VIP `10.1.30.58`; AP controller access uses the three explicit rules below. |
| 16-18 | `sk-firewall/forward/allow-unifi-ap-inform`, `sk-firewall/forward/allow-unifi-ap-stun`, `sk-firewall/forward/allow-unifi-ap-discovery` | Allow VLAN 102 APs to the UniFi device-communication VIP only on TCP/8080, UDP/3478, and UDP/10001; stateful return traffic uses the established/related rule. |
| 19-20 | `sk-firewall/forward/allow-wireguard-kubernetes-dns-udp`, `sk-firewall/forward/allow-wireguard-kubernetes-dns-tcp` | Allow only the verified road-warrior addresses to use the Kubernetes DNS VIP. |
| 21 | `sk-firewall/forward/allow-smtp-relay-from-printer` | Allow only printer `10.1.10.250/32` to submit SMTP over TCP/587 to relay VIP `10.1.30.58`. |
| 22-25 | Existing Kubernetes SNMP rules | Preserve the narrow Synology and UniFi request and reply paths; the imported Synology reply rule remains enabled until post-apply counter testing proves it redundant. |
| 26 | `sk-firewall/forward/allow-kubernetes-proxmox` | Allow the Kubernetes worker VLAN to reach the Proxmox API at `10.1.100.201:8006`; this is the exporter’s only routed management exception. |
| 27 | `sk-firewall/forward/allow-kubernetes-dell` | Allow the Kubernetes worker VLAN to reach the Dell Server’s Proxmox-compatible API at `10.1.100.202:8006`, using the same narrow rule as the original node. |
| 28-29 | `sk-firewall/forward/allow-management-proxmox`, `sk-firewall/forward/allow-management-dell` | Allow VLAN 10 administrators to reach the Proxmox-compatible APIs at `10.1.100.201:8006` and `10.1.100.202:8006` with identical narrow TCP rules. |
| 30 | `forward_management` | Empty by default; new inter-VLAN management requires a commented map entry. |
| 31 | `sk-firewall/forward/allow-wan-dstnat` | Preserve only new WAN flows that matched the active destination NAT rule. |
| 32 | `sk-firewall/forward/drop-inter-vlan` | Drop unauthorized trusted-LAN to trusted-LAN forwarding. |
| 33 | `sk-firewall/forward/drop-wan-inbound` | Drop new WAN-to-LAN flows that are not destination-NATed. |
| 34 | `sk-firewall/forward/drop-unmatched` | Drop every remaining forwarded packet. |
| 35+ | Retired and other imported rollback rules | Remain after the terminal deny and cannot provide an allow path; every captured identity remains state-owned. The generated FastTrack dummy is state-owned separately and is not moved. |
...
For UniFi AP management recovery, use the narrower mutually exclusive `plan_gateway_unifi_ap=true` path from `main`. It targets only three forward accepts from `10.1.102.0/24` entering `vlan102` to the UniFi device-communication VIP `10.1.30.1`: TCP/8080 for inform, UDP/3478 for STUN, and UDP/10001 for discovery, plus the existing `sk-firewall/forward/drop-inter-vlan` anchor. The targeted plan does not target or apply the broader Kubernetes VIP policy; that separate full firewall plan owns the declarative exclusion of `10.1.102.0/24`, and VLAN 102 remains outside live `LAN` and broad VIP access until that plan is applied. The plan guard accepts exactly the three AP-rule creates on initial recovery or an empty review plan after recovery, and rejects deletes, replacements, or unrelated resources. Review `network-gw-unifi-ap-tofuplan`, then run a separate `apply_gateway_unifi_ap=true` dispatch; that trusted run regenerates and uploads its own immutable artifact from current `main`, so inspect it and approve production only when the same exact three-create guard passes. The production-gated job applies that artifact and verifies exactly one enabled rule per comment, the exact fields and ports, the inform/STUN/discovery order, and both enabled forward deny anchors. DHCP may advertise `10.1.30.53` to VLAN 102, but that does not grant DNS or general Kubernetes VIP forwarding; only the three UniFi controller ports are allowed for AP management. Re-run the review-only dispatch afterward and require an empty plan. Do not add VLAN 102 to `LAN`, grant broad access, or use the broader firewall path for this fix.

If AP verification fails, do not retry the artifact or edit RouterOS through REST. Use RouterOS Safe Mode or the local console only to disable the three new rules identified by their stable `sk-firewall/forward/allow-unifi-ap-*` comments, then correct the declaration and create a fresh reviewed targeted plan. Stateful return traffic remains covered by the existing established/related forward rule; no reverse-direction or NAT rule is part of this exception.

### Rollback

If the review plan is wrong, do not apply its artifact. If a live acceptance probe fails after apply, use RouterOS Safe Mode through the VLAN 100 management path or local console and disable only the affected managed rule or rules identified by their stable `sk-firewall/...` comments. For this AP fix, those comments are `sk-firewall/forward/allow-unifi-ap-inform`, `sk-firewall/forward/allow-unifi-ap-stun`, and `sk-firewall/forward/allow-unifi-ap-discovery`; never disable a terminal drop rule as the rollback. This is a break-glass recovery action, not the normal ownership path: do not use REST, do not change unrelated rules, and record the temporary change. Correct the non-secret `firewall_policy` declaration and run a new reviewed targeted plan. A full declaration revert must be a separately reviewed change; the normal no-destroy guard intentionally refuses rollback artifacts that delete managed resources.

The pre-change baseline and read-only physical checks do not establish full-speed acceptance. The live check confirmed ether3 is a forwarding, hardware-offloaded bridge port with PVID 100, VLAN 100 current-untagged membership, bridge firewall processing disabled, one complete ARP owner for `10.1.100.10`, and no DHCP lease for that address. Run representative acceptance tests from their actual source networks after the policy apply: reach representative ICMP, TCP, and UDP Synology services from VLANs 10, 20, 101, and 102; verify VLAN 100 Layer-2 access; verify DHCP leases and advertised DNS separately from forwarding, allowing VLAN 102 to receive the Kubernetes DNS address but requiring DNS and other Kubernetes VIP forwarding from VLAN 102 to remain denied except for the three UniFi controller ports; confirm the matching Synology allow and established/FastTrack counters increment without terminal-drop counters; run three 60-second, four-stream `iperf3` tests in both directions from each routed VLAN and require at least 90% of the slowest negotiated endpoint link without material loss or router CPU saturation; resolve and reach the gateway DNS service from VLANs 10, 20, and 100; resolve `10.1.30.53` over both UDP and TCP 53 from a verified road-warrior client; reach TCP/22 and TCP/443 from VLAN 10 and VLAN 100 management hosts; establish both WireGuard listeners from their WAN peers; reach the Kubernetes VIP and the SNMP request path from the worker VLAN; submit SMTP over TCP/587 from printer `10.1.10.250` to relay VIP `10.1.30.58`; verify trusted LAN egress and the existing WAN destination-NAT service; and confirm that other printer-to-VLAN traffic, SMTP relay access from an unapproved source, an unapproved VLAN-to-VLAN connection, a road-warrior connection to a non-DNS Kubernetes VIP, a WAN connection without destination NAT, an unknown TCP/179 source, and an unknown WireGuard source are denied. Record only pass/fail, source class, destination class, throughput, packet-loss, CPU, and the final managed rule order; never record credentials or raw API responses.
...
The inventory places Synology on `ether3`, untagged VLAN 100, at `10.1.100.10`. VLAN 100 is a layer-2 path and does not traverse the forward chain; VLANs 10, 20, 101, and 102 are routed paths. OpenTofu preserves the shared-NAS allow for every addressed internal VLAN through `sk-internal-vlan-networks` and `sk-internal-vlans`, while also deriving explicit routed Synology rules from the authoritative VLAN inventory. The NAS policy precedes the inter-VLAN, WAN-inbound, and terminal-unmatched drops. Established sessions are still FastTracked after the initial allow, so the exception does not add per-packet logging or a throughput limiter.

The existing `lan-to-nas` rule remains enabled as the canonical shared-NAS path, but its source is constrained to the internal VLAN address/interface boundary so WAN ingress cannot use it. The move sequence places it, the VLAN 10 management exception, and the VLAN 10 ping exception before `sk-firewall/forward/drop-unmatched`; the four exact Synology rules remain explicit ordered policy for the routed VLANs.

Do not add VLAN 101 or VLAN 102 to the broad `LAN` interface list to solve this path. The implementation uses `sk-internal-vlans` for DHCP and the Kubernetes service-VIP boundary, but that broad VIP rule excludes VLAN 102; exact rules then grant only the reviewed AP controller ports and other declared paths. General WAN egress and broad inter-VLAN access remain outside that list.
