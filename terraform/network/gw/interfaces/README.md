...
For UniFi AP management recovery, use the narrower mutually exclusive `plan_gateway_unifi_ap=true` path from `main`. It targets the broad Kubernetes VIP rule update that excludes `10.1.102.0/24`, plus three forward accepts from that subnet entering `vlan102` to the UniFi device-communication VIP `10.1.30.1`: TCP/8080 for inform, UDP/3478 for STUN, and UDP/10001 for discovery. The plan guard accepts exactly those three creates plus the one exact VIP-boundary update on first apply or an empty plan after recovery, and rejects deletes, replacements, or unrelated resources. Review `network-gw-unifi-ap-tofuplan`, then run a separate `apply_gateway_unifi_ap=true` dispatch; that trusted run regenerates and uploads its own immutable artifact from current `main`, so inspect it and approve production only when the same exact guard passes. The production-gated job applies that artifact and verifies exactly one enabled rule per comment, the exact fields and ports, the inform/STUN/discovery order, and the Kubernetes VIP exclusion before both enabled forward deny anchors. DHCP may advertise `10.1.30.53` to VLAN 102, but that does not grant DNS or general Kubernetes VIP forwarding; only the three UniFi controller ports are allowed for AP management. Re-run the review-only dispatch afterward and require an empty plan. Do not add VLAN 102 to `LAN`, grant broad access, or use the broader firewall path for this fix.
