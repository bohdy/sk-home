...
- A separate twice-daily `terraform-drift` workflow checks the current drift-enabled stacks for drift with `terraform plan -detailed-exitcode`, keeps the Terraform CLI wrapper disabled so exit code `2` remains visible to the shell, uploads the plain-text plan when drift is found with a 1-day artifact retention window, sends a concise Telegram message for the affected stack, and fails the run so the drift is visible in Actions. Today that includes `network-core`, `network-core/interfaces/gw`, `network-core/interfaces/switch-1pp`, `network-core/interfaces/switch-1np`, `network-core/dhcp`, `network-core/routing`, `platform-k3s`, and `dns-blocky-k3s`.
