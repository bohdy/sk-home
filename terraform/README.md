...
- `stacks/dns-blocky-k3s/`: Blocky DNS workload for the new k3s cluster with HA DaemonSet and metrics endpoint
...
- `stacks/platform-k3s/`: k3s Kubernetes cluster on Flatcar Container Linux VMs (Proxmox), provisioned via Butane/Ignition and k3s sysext
- `stacks/wifi/`: documented bootstrap root reserved for future UniFi wireless configuration
...
- GitHub Actions separately runs repo-hygiene validation for tracked Markdown, shell scripts, and workflow syntax so repo-wide maintenance changes are checked even when they do not touch a Terraform stack.
...
If `tflint` itself starts but then fails during plugin initialization, treat that as a local tooling-health problem rather than a Terraform-stack regression. Reinstall or repair the local `tflint` package before relying on commit-time lint results.
...
- The `wifi` stack intentionally stays validate-only until real wireless resources and provider requirements are introduced.
...
- A separate twice-daily `terraform-drift` workflow checks the current drift-enabled CI-ready stacks for drift with `terraform plan -detailed-exitcode`, keeps the Terraform CLI wrapper disabled so exit code `2` remains visible to the shell, uploads the plain-text plan when drift is found with a 1-day artifact retention window, sends a concise Telegram message for the affected stack, and fails the run so the drift is visible in Actions. Today that includes `network-core`, `network-core/interfaces/gw`, `network-core/interfaces/switch-1pp`, `network-core/interfaces/switch-1np`, `network-core/dhcp`, `network-core/routing`, and `platform-k3s`.
...
- The drift workflow now uses workflow concurrency cancellation, a 45-minute job timeout, a two-stack matrix parallelism cap, explicit step timeouts, and a shared Terraform plugin cache so slow provider refreshes or unreachable endpoints do not occupy the self-hosted runner indefinitely.
...
- Terraform validation runs also use bounded concurrency and step timeouts, while applies run one stack at a time with per-stack concurrency protection so state-changing jobs do not overlap accidentally.
- The separate `repo-hygiene` workflow checks tracked Markdown normalization, shell-script syntax, and GitHub Actions workflow syntax on pull requests and qualifying branch pushes.
...
- Bitwarden should store `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `MIKROTIK_USERNAME`, `MIKROTIK_PASSWORD`, `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, `KUBECONFIG_CONTENT`, `MIKROTIK_SSH_PRIVATE_KEY`, `MIKROTIK_SSH_KNOWN_HOSTS`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, and `K3S_TOKEN` for this repo.
