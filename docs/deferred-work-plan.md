...
- [ ] Resolve the RouterOS provider incompatibility affecting the worker BGP peer, then apply and verify the declarative peer through the production-gated workflow; do not use an imperative REST workaround.
- [ ] Separate untrusted pull-request OpenTofu validation from trusted production planning and applying so PRs receive no infrastructure credentials and sensitive binary plans are not retained as ordinary artifacts.
