...
Both Blocky and CoreDNS should run two replicas. Replicas should be spread across nodes with topology spread on `kubernetes.io/hostname`. DNS pods may run on any stable Linux node, including general-purpose workers and control-plane nodes. DNS should not require any single worker because losing one worker must not also remove the internal DNS path.
