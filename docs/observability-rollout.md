...
- A dedicated endpoint-scoped read-only UniFi Network credential is injected from the `unifi-poller-auth` Secret in `observability`; the deployment reads its `username` and `password` keys, and neither value is committed to Git, ConfigMaps, pod arguments, or logs.
