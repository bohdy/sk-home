...
- The accepted dashboard set has eleven focused, repository-owned Grafana dashboards covering network interfaces, APC UPS, Synology, Proxmox, DNS, ingestion health, Cilium/BGP, syslog, flow collection, UniFi controller health, and the SMTP relay without runtime dashboard downloads. The UniFi Poller dashboard's Bitwarden-backed Secret, optional Flux child, verified LAN TLS collection, healthy VMAgent targets, device-series filter, and inactive VMAlert rule were accepted without exposing credential values. The exporter hashes client names and MAC addresses, and the scrape removes `mac`, `name`, `hostname`, and `ip` labels before storage, so stored client series retain aggregate/client-quality values without those identifying labels. The operator confirmed that the dedicated local `unifi-poller-api` account is endpoint-scoped and read-only; no credential values are recorded here.
...
- On 2026-09-03 after PR #335, Flux applied `main@sha1:30b8f7a3fd9c29d8c191a13e93758f2f8fc158ee`, the generated dashboard ConfigMap contained all eleven committed JSON files, and Grafana's authenticated API returned the provisioned `sk-smtp-relay` dashboard with six panels.
