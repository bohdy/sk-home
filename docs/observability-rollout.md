...
Flow acceptance completed on 2026-08-07 for the active NodePort path: RouterOS exported `ether8` and every routed VLAN through worker-1 at `10.1.20.44:31236`, ClickHouse contained active `flows.flow` and `flows.flow_analytics` data, and the GeoIP dictionary tables were loaded. A test cutover to the Cilium collector VIP did not sustain new records and was reverted; worker-failure-tolerant IPFIX transport remains deferred until the CP-to-worker path is fixed. The post-recovery GeoIP refresh still needs one successful run after the ClickHouse memory and RWO rollout fixes.
