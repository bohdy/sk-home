...
- The accepted dashboard baseline has eight focused, repository-owned Grafana dashboards covering network interfaces, APC UPS, Synology, Proxmox, DNS, ingestion health, Cilium/BGP, and syslog without runtime dashboard downloads. The UniFi Poller adds a ninth prepared dashboard for controller-side health and client summaries, but its Secret bootstrap, Flux reconciliation, and live Grafana/VMAlert acceptance remain pending.
