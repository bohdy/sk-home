...
- Initial live reconciliation showed that both supported container entrypoints require `CHOWN` to assign their mounted data paths before they drop to their configured users. The private-stage security contexts therefore drop every capability except `CHOWN`; no broad privileged exception is used.
