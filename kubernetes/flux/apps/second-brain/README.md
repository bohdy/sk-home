...
2. Verify live Flux, Cilium, `synology-iscsi-retain`, storage capacity, PVC ownership behavior, DNS, actual cluster/service/node CIDRs, and network-policy enforcement. A read-only live baseline on 2026-09-20 found 6 Ready amd64 nodes, Flux/Cilium/CSI Ready, and 8 PVCs Bound; free NAS capacity remains unknown. Cilium 1.19.4 showed both live cloudflared CiliumEndpoints with exactly these identity labels: k8s:io.cilium.k8s.policy.serviceaccount=cloudflared, k8s:io.kubernetes.pod.namespace=cloudflare-tunnel, and k8s:app.kubernetes.io/name=cloudflared; the tunnel selector below matches the installed identity. Test permitted public IPv4 HTTP/HTTPS and denied private, metadata, and special-use destinations, including redirects and the reviewed exclusions `192.31.196.0/24`, `192.52.193.0/24`, `192.88.99.0/24`, and `192.175.48.0/24`.
