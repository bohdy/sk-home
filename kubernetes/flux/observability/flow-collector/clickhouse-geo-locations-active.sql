-- Store the English MaxMind location rows used to attach country and city
-- names to the network ranges imported by the refresh job.
CREATE TABLE IF NOT EXISTS flows.geoip_locations_active (
    geoname_id UInt32,
    locale_code LowCardinality(String),
    continent_code LowCardinality(String),
    continent_name String,
    country_iso_code LowCardinality(String),
    country_name String,
    subdivision_1_iso_code LowCardinality(String),
    subdivision_1_name String,
    subdivision_2_iso_code LowCardinality(String),
    subdivision_2_name String,
    city_name String,
    metro_code String,
    time_zone String,
    is_in_european_union Nullable(UInt8)
) ENGINE = MergeTree()
ORDER BY geoname_id;
