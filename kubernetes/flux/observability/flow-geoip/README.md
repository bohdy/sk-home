...
The updater uses HTTPS with certificate validation, follows MaxMind's documented R2 redirect, retries transient download failures within a bounded window, allows egress only to the two required MaxMind names, and never sends individual flow addresses to MaxMind. The downloaded CSV files are staged in ephemeral storage and are not retained outside ClickHouse.
