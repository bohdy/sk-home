...
The official distroless image declares UID 0, and the Agent needs write access to its node-local hostPath checkpoint and buffer directory. The container therefore runs as UID 0, cannot escalate privileges, uses a read-only root filesystem, and uses RuntimeDefault seccomp. It receives only `DAC_READ_SEARCH`, which Talos requires to read kube-apiserver audit files written by the `nobody` user.
...
TCP and UDP syslog are throttled independently at 1,000 events per second for each preserved sender address. This leaves a tenfold reserve above the initial 100 events/second planning load while bounding a runaway device. Vector omits sender keys from internal metrics to avoid attacker-controlled cardinality. Vector 0.57.0 does not emit a discarded-event series for throttle drops, so alerting must derive the aggregate drop count from the difference between the throttle component's received and sent event counters.

Talos service and kernel logs use JSON lines over TCP hostPort 6051 on each node's own management address, bypassing the syslog LoadBalancer. Vector parses that stream separately from RFC syslog, preserves malformed records, assigns `talos_stream` as `service`, `kernel`, or `unknown`, and keeps Talos' sender timestamp separate from its receipt timestamp. The Talos OpenTofu stack supplies a stable node tag for service logs. Talos kernel records contain no node tag, so direct node-local delivery lets those records use the receiving Vector Agent's Downward API node name as their stable device identity.

Talos writes kube-apiserver audit JSON lines under `/var/log/audit/kube`. Vector mounts only that directory read-only, follows the active `kube-apiserver.log` across rotation, and checkpoints file offsets in its existing node-local data directory. Timestamped archives are intentionally excluded from initial discovery so enabling collection does not backfill several gigabytes of already-rotated events. The explicit Metadata policy records authentication, authorization, RBAC, secret-access, and workload-mutation outcomes without request or response bodies. Normalized identity keeps the authenticated caller in `username` and any authorization subject in `impersonated_username`. The transform defensively removes `requestObject` and `responseObject` if either appears, and it omits the original text of malformed audit records because unparseable content cannot be scrubbed safely.
