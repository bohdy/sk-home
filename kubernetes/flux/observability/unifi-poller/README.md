...
The `VMServiceScrape` drops every `unpoller_device_*` series so AP device coverage remains SNMP-owned while controller, site, and `unpoller_client_*` metrics remain available. Before storage, the scrape removes labels named `mac`, `name`, `hostname`, and `ip` from every series, so no stored client series contains those identifying labels; metric names, sample values, and aggregate labels such as `site_name` remain available for client-quality panels. The Pod maps canonical hostname `unifi.bohdal.name` to the cert-manager-backed `unifi-console-lan` Service's fixed `10.1.30.56` LAN VIP, while the exporter still connects to `https://unifi.bohdal.name:443` with certificate verification enabled. Its Cilium policy permits only the Service's `10.1.30.56/32:443` VIP and the selected TLS proxy backend on `8443`; it does not permit direct controller access. The dashboard's connected-client panel uses `unpoller_client_rssi_db` and remains populated under this contract.
