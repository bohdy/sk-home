...
Grafana and Alertmanager use retained 10 GiB and 1 GiB claims respectively. Grafana's Helm-managed claim carries the `helm.sh/resource-policy: keep` annotation so failed-install remediation and intentional Helm removal preserve it. The datasource sidecar loads the labeled cleanup ConfigMap over Grafana's local HTTPS reload endpoint, uses request-specific TLS verification bypass for the local certificate, removes stale ClickHouse records, and leaves the IaC-owned `Flow ClickHouse IaC` datasource with UID `FlowClickHouseIaC`.
