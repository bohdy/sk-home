...
Grafana and Alertmanager use retained 10 GiB and 1 GiB claims respectively. Grafana's Helm-managed claim carries the `helm.sh/resource-policy: keep` annotation so failed-install remediation and intentional Helm removal preserve it. Grafana's dashboard and datasource sidecars reload through the local HTTPS listener using the certificate-valid canonical hostname mapped to loopback; the datasource sidecar uses its own health listener, removes stale ClickHouse records, and leaves the IaC-owned `Flow ClickHouse IaC` datasource with UID `FlowClickHouseIaC`.

Grafana installs the pinned `victoriametrics-logs-datasource@0.29.0`, `grafana-clickhouse-datasource@4.20.0`, and `netsage-sankey-panel@1.1.4` plugins. The native Geomap and State timeline visualizations cover the requested map and timeline views, so the deprecated `grafana-worldmap-panel` is intentionally not installed.
