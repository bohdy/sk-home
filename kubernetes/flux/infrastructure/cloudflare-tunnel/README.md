...
The Cloudflare control-plane stack routes `grafana.bohdal.name` and `unifi.bohdal.name` through these connectors and leaves a terminal `404` rule for every unmatched hostname. Deploying the connector alone still cannot publish an application; public DNS, tunnel ingress, and Access remain OpenTofu-owned resources.
