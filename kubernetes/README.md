...
The `base` component owns the shared namespace, its resource defaults, and its aggregate capacity quota before any namespace-scoped observability resource. The reusable `snmp` component pins SNMP Exporter and its reviewed generated vendor modules, with the accepted MikroTik target enabled and remaining devices gated by the explicit inventory and Bitwarden-backed credentials. The independent `blackbox` component performs confirmed gateway ICMP, Blocky DNS, Grafana HTTPS, and stable external HTTPS probes through VMAgent. The `alerting` component adds missing Flux and cert-manager coverage plus sustained local rules without duplicating the stack defaults. The `grafana-tls` component issues Grafana's production certificate before its listener and Service are exposed. Proxmox export and Discord notification delivery remain later dependency-ordered work.
