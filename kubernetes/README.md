...
The `base` component owns the shared namespace before any namespace-scoped observability resource. The reusable `snmp` component pins SNMP Exporter and its reviewed generated vendor modules, but remains outside the live Flux tree until the explicit target inventory and Bitwarden-backed credential Secret are complete. The independent `blackbox` component performs confirmed gateway ICMP and Blocky DNS probes through VMAgent. The `alerting` component adds missing Flux and cert-manager coverage plus sustained local rules without duplicating the stack defaults. The `grafana-tls` component issues Grafana's production certificate before its listener and Service are exposed. Proxmox export, remaining synthetic targets, public Grafana routing, and notification delivery remain later dependency-ordered work.
