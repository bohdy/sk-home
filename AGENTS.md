...
- RouterOS changes that must avoid known provider-broken resources require a dedicated, mutually exclusive workflow input that plans explicit targets, rejects destructive firewall artifacts where applicable, uploads the immutable plan, and applies only that artifact through the production environment.
