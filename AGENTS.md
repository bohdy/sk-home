...
- The optional RouterOS qnetd quorum device is disabled by default and follows its own main-only, mutually exclusive targeted workflow; trusted runs derive only the Proxmox SSH public key from the existing private key in ephemeral runner files, adopt and validate the existing VLAN attachment before planning, fail closed on unsafe global container configuration, reject destructive or unrelated changes, add no firewall rules for the same-VLAN Layer-2 path, and verify the USB-backed container after the production-gated immutable apply.
