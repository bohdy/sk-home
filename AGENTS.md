...
- Creation-only credentials generated by OpenTofu must be marked sensitive, retained only in encrypted remote state, masked before any workflow use, and handed directly to a dedicated Bitwarden item. The workflow must validate non-secret identity structure and fail if the handoff fails.
